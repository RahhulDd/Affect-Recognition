# -*- coding: utf-8 -*-
"""
Created on Thu Dec 10 12:11:07 2020
...
class _DatasetBase(Dataset):
    def __init__(self,phase,config):
        super(_DatasetBase,self).__init__()
        ...
        if phase == 'Train':
            self.character_names = ['ale_pt','ali_pt','alx_pt','amc_pt','bar_pt',
                                    'boo_pt','chr_pt','dav_pt','din_pt','dun_pt',
                                    'ele_pt','emm_pt','gra_pt','ian_pt','jan_pt',
                                    'jen_pt','jua_pt','kat_pt','lin_pt','mac_pt',
                                    'mar_pt','mil_pt','ndy_pt','pet_pt','rac_pt']
            self.aug = True
        else:
            self.character_names = ['ros_pt', 'she_pt', 'shi_pt', 'ste_pt', 'vas_pt']
            self.aug = False

        #items = glob.glob(os.path.join(self.data_root, self.character_names[0], '*/*/*/motions/*.npy'))
        #self.motion_names = ['\\'.join(x.split('\\')[-5:]) for x in items]
        self.items = [glob.glob(os.path.join(self.data_root,self.character_names[i],'*/*/*/motions/*.npy')) for i in range(len(self.character_names))]
        self.motion_names = ['\\'.join(i.split('\\')[-5:]) for x in self.items for i in x]
